        ...
        while (true) {
            ...
            if (token == FIELD_NAME || token == START_OBJECT) {
                if (context.testExpression("crawlerLimits", targetDepth)) {
                    context.nextToken();
                    webCrawlerConfiguration.setCrawlerLimits(WebCrawlerLimitsJsonUnmarshaller.getInstance().unmarshall(context));
                }
                if (context.testExpression("exclusionFilters", targetDepth)) {
                    context.nextToken();
                    webCrawlerConfiguration.setExclusionFilters(new ListUnmarshaller<String>(context.getUnmarshaller(String.class))

                    .unmarshall(context));
                }
                if (context.testExpression("inclusionFilters", targetDepth)) {
                    context.nextToken();
                    webCrawlerConfiguration.setInclusionFilters(new ListUnmarshaller<String>(context.getUnmarshaller(String.class))

                    .unmarshall(context));
                }
                if (context.testExpression("scope", targetDepth)) {
                    context.nextToken();
                    webCrawlerConfiguration.setScope(context.getUnmarshaller(String.class).unmarshall(context));
                }
